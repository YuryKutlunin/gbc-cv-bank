...
export const Certificate = (props: ICertificateProps) => {
  ...
  const { certificateList, match, loading } = props;
  return (
    <div>
      <h2 id="certificate-heading">
        Certificates
                ...
                <tr key={`entity-${i}`}>
                  ...
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`${match.url}/${certificate.id}`} color="info" size="sm">
                        <FontAwesomeIcon icon="eye" /> <span className="d-none d-md-inline">View</span>
                      </Button>
                      <Button tag={Link} to={`${match.url}/${certificate.id}/edit`} color="primary" size="sm">
                        <FontAwesomeIcon icon="pencil-alt" /> <span className="d-none d-md-inline">Edit</span>
                      </Button>
                      <Button tag={Link} to={`${match.url}/${certificate.id}/delete`} color="danger" size="sm">
...
const mapStateToProps = ({ certificate }: IRootState) => ({
  certificateList: certificate.entities,
