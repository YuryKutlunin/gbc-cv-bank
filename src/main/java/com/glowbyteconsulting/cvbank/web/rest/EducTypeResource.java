...
/**
 * REST controller for managing {@link com.glowbyteconsulting.cvbank.domain.EducType}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class EducTypeResource {
    ...
    public ResponseEntity<EducType> createEducType(@RequestBody EducType educType) throws URISyntaxException {
        ...
        return ResponseEntity
            .created(new URI("/api/educ-types/" + result.getId()))
    ...
    public ResponseEntity<EducType> updateEducType(@RequestBody EducType educType) throws URISyntaxException {
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, educType.getId().toString()))
            .body(result);
    }
