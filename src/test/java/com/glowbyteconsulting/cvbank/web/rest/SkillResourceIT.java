...
public class SkillResourceIT {
    ...
    public static Skill createEntity(EntityManager em) {
        Skill skill = new Skill().idSkill(DEFAULT_ID_SKILL).skillNm(DEFAULT_SKILL_NM);
        return skill;
    }
    ...
    public static Skill createUpdatedEntity(EntityManager em) {
        Skill skill = new Skill().idSkill(UPDATED_ID_SKILL).skillNm(UPDATED_SKILL_NM);
        return skill;
    }
    ...
    public void createSkill() throws Exception {
        ...
        // Create the Skill
        restSkillMockMvc
            .perform(post("/api/skills").contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(skill)))
    ...
    public void getAllSkills() throws Exception {
        ...
        restSkillMockMvc
            .perform(get("/api/skills?sort=id,desc"))
    ...
    public void getSkill() throws Exception {
        ...
        // Get the skill
        restSkillMockMvc
            .perform(get("/api/skills/{id}", skill.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
    ...
    public void getNonExistingSkill() throws Exception {
        // Get the skill
        restSkillMockMvc.perform(get("/api/skills/{id}", Long.MAX_VALUE)).andExpect(status().isNotFound());
    }
    ...
    public void updateSkill() throws Exception {
        ...
        updatedSkill.idSkill(UPDATED_ID_SKILL).skillNm(UPDATED_SKILL_NM);

        restSkillMockMvc
            .perform(put("/api/skills").contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(updatedSkill)))
            .andExpect(status().isOk());
    ...
    public void updateNonExistingSkill() throws Exception {
        ...
        restSkillMockMvc
            .perform(put("/api/skills").contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(skill)))
            .andExpect(status().isBadRequest());
    ...
    public void deleteSkill() throws Exception {
        ...
        // Delete the skill
        restSkillMockMvc
            .perform(delete("/api/skills/{id}", skill.getId()).accept(MediaType.APPLICATION_JSON))
