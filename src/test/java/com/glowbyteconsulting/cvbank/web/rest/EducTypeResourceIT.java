...
import com.glowbyteconsulting.cvbank.repository.EducTypeRepository;
...
public class EducTypeResourceIT {
    ...
    public static EducType createEntity(EntityManager em) {
        EducType educType = new EducType().idEducType(DEFAULT_ID_EDUC_TYPE).educTypeNm(DEFAULT_EDUC_TYPE_NM);
        return educType;
    }
    ...
    public static EducType createUpdatedEntity(EntityManager em) {
        EducType educType = new EducType().idEducType(UPDATED_ID_EDUC_TYPE).educTypeNm(UPDATED_EDUC_TYPE_NM);
        return educType;
    }
    ...
    public void createEducType() throws Exception {
        ...
        // Create the EducType
        restEducTypeMockMvc
            .perform(post("/api/educ-types").contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(educType)))
    ...
    public void getAllEducTypes() throws Exception {
        ...
        restEducTypeMockMvc
            .perform(get("/api/educ-types?sort=id,desc"))
    ...
    public void getEducType() throws Exception {
        ...
        // Get the educType
        restEducTypeMockMvc
            .perform(get("/api/educ-types/{id}", educType.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
    ...
    public void getNonExistingEducType() throws Exception {
        // Get the educType
        restEducTypeMockMvc.perform(get("/api/educ-types/{id}", Long.MAX_VALUE)).andExpect(status().isNotFound());
    }
    ...
    public void updateEducType() throws Exception {
        ...
        updatedEducType.idEducType(UPDATED_ID_EDUC_TYPE).educTypeNm(UPDATED_EDUC_TYPE_NM);

        restEducTypeMockMvc
            .perform(
                put("/api/educ-types").contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(updatedEducType))
            )
            .andExpect(status().isOk());
    ...
    public void updateNonExistingEducType() throws Exception {
        ...
        restEducTypeMockMvc
            .perform(put("/api/educ-types").contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(educType)))
            .andExpect(status().isBadRequest());
    ...
    public void deleteEducType() throws Exception {
        ...
        // Delete the educType
        restEducTypeMockMvc
            .perform(delete("/api/educ-types/{id}", educType.getId()).accept(MediaType.APPLICATION_JSON))
